...
public class Unit : WorldObject {
    ...
    public virtual void SetCreator(Building creator) {
    }
    ...
    public override bool MouseClick(GameObject hitObject, Vector3 hitPoint, Player controller) {
        if (base.MouseClick(hitObject, hitPoint, controller))
            return true;
        //only handle input if owned by a human player and currently selected
        if (player && player.human && currentlySelected && controllable) {
